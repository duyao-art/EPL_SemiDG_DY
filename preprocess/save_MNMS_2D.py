...
# transform the labeled slice as RGB channel information for label representation.
...
# save the original data as grayscale image.
...
for num_pat in range(0, len(LabeledVendorA_names)):
    ...
    # hide, width, time-frame, number-slice
...
for num_pat in range(0, len(LabeledVendorA_names)):
    ...
    for num_row in range(1, 346):
        if sheet.cell_value(num_row, 1) == gz_name[0][0:6]:
            ED = sheet.cell_value(num_row, 5)
            ES = sheet.cell_value(num_row, 6)
            break
